...
// }
/*********************************************************************************************************************************************** */
//updated code with permission issues
// package com.example.sdk_connection_2;
...
//     bluetoothGatt.readCharacteristic(activeCharacteristic);
//     activeCharacteristic.setValue(activeCharacteristic.getValue());  // Trigger a read if needed
//     bluetoothGatt.readCharacteristic(activeCharacteristic);

//     // After reading the characteristic, send data to Flutter
//     result.success("Reading weight data...");
//     byte[] weightData = activeCharacteristic.getValue();
//     Map<String, Object> weightInfo = new HashMap<>();
//     weightInfo.put("weight", weightData);
//     new MethodChannel(getFlutterEngine().getDartExecutor().getBinaryMessenger(), CHANNEL)
//         .invokeMethod("onWeightDataReceived", weightInfo);
// }
...
public class MainActivity extends FlutterActivity {
    ...
    public void onConnectionStateChange(BluetoothGatt gatt, int status, int newState) {
        if (newState == BluetoothGatt.STATE_CONNECTED) {
            bluetoothGatt = gatt;
            bluetoothGatt.discoverServices(); // Trigger service discovery
        } else if (newState == BluetoothGatt.STATE_DISCONNECTED) {
            sendToFlutter("onConnectionStateChange", "Disconnected from device.");
        }
    }
    ...
    public void onServicesDiscovered(BluetoothGatt gatt, int status) {
        if (status == BluetoothGatt.GATT_SUCCESS) {
            ...
            // Send discovered UUIDs to Flutter
            sendToFlutter("onServicesDiscovered", serviceAndCharacteristicMap);
        } else {
            sendToFlutter("onServicesDiscoveredError", "Failed to discover services.");
        }
    }
};
